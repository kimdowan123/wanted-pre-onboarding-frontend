    ...
    return (
        <div className={ modal === true ? 'BlackModal' : 'Disabled-BlackModal'} >
            <div className='whiteModal'>
                ...
                <h2>회원가입</h2>
                <div className='alert'>
                    <p>* 아이디</p> <p className={IdCheck === true ? 'correct' : 'non-correct'}>{IdMessage}</p>
                </div>
                <input type="text" placeholder='Email을 입력해주세요.' onInput={(e) => {
                    EmailCheck(e)
                }} />

                <div className='alert'>
                    <p>* 비밀번호</p> <p className={PwCheck === true ? 'correct' : 'non-correct'}>{PwMessage}</p>
                </div>
                <input type="password" placeholder="Password를 입력해주세요." onInput={(e) => {
                    passWordCheck(e)
                }} />

                <button className={activateBTN === true ? 'disabled-accountBTN' : 'accountBTN'} disabled={activateBTN} onClick={() => {
